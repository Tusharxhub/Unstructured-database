...
(async () => {
  ...
  try {
    ...
    await col.insertMany([...Array(N)].map((_, i) => ({
      studentId: i + 1,
      name: `Student_${i + 1}`,
    ...
    const run = async (label, q) => {
      const t = Date.now(), e = await col.find(q).explain("executionStats");
      console.log(label, { stage: s(e.executionStats.executionStages), docs: e.executionStats.totalDocsExamined, ms: Date.now() - t });
    };
    ...
    await run("EXPRESS IXSCAN", { studentId: 50000 });
